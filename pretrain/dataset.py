...
def normalize_features(
    ...
    peak_normalization: str = "log_quantile",
) -> np.ndarray:
    """Normalize peak intensities within each modality; keep ratios unchanged."""
    values = np.asarray(values, dtype=np.float32).copy()
    peak_mask = np.asarray(peak_mask, dtype=bool)
    scales = np.maximum(np.asarray(peak_scales, dtype=np.float32)[peak_mask], 1e-8)
