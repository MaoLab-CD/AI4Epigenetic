...
from model import build_feature_metadata
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--mask_strategy", "--mask-strategy", dest="mask_strategy",
        choices=["random", "modality"], default="modality",
        help="Mask independent features or all features related to chosen modalities",
    )
    ...
    parser.add_argument(
        "--peak_normalization", "--peak-normalization",
        dest="peak_normalization",
        choices=["log_quantile", "linear_quantile"],
        default="log_quantile",
        help=("Per-modality peak scaling. log_quantile is robust to long-tailed "
              "sequencing signals; linear_quantile preserves linear fold ratios."),
    )
    parser.add_argument(
        "--embedding_dropout", "--embedding-dropout",
        dest="embedding_dropout", type=float, default=0.1,
    )
    parser.add_argument(
...
def infer_companion_path(data: Path, suffix: str) -> Path:
    stem = data.name.removesuffix("_feature_matrix.h5")
...
def write_feature_table(path: Path, selection) -> None:
    metadata = build_feature_metadata(selection.names)
    typed = {
        int(source_index): parsed
        for source_index, parsed in zip(
            selection.source_indices, metadata["parsed"]
        )
    }
    selected = {
    ...
    with path.open("w", encoding="utf-8", newline="") as handle:
        ...
        writer.writerow([
            "model_index", "source_index", "name", "status",
            "denominator_modality", "numerator_modality", "data_form",
        ])
        for source_index in sorted(selected.keys() | excluded.keys()):
            if source_index in selected:
                model_index, name = selected[source_index]
                primary, partner, form = typed[source_index]
                writer.writerow([
                    model_index, source_index, name, "selected",
                    primary, partner or "-", form,
                ])
            else:
                writer.writerow([
                    "-", source_index, excluded[source_index], "excluded",
                    "-", "-", "-",
                ])
...
def main(args: argparse.Namespace) -> None:
    ...
    if not 0.0 <= args.embedding_dropout < 1.0:
        raise ValueError("--embedding-dropout must be in [0, 1)")
    if args.save_freq <= 0:
    ...
    if len(selection.names) != 288 or len(selection.excluded_names) not in {0, 35}:
        raise RuntimeError(
            "Expected 288 selected features and either 0 (current direct schema) "
            "or 35 (legacy full schema) PolIIS5P-related excluded features, "
    ...
    if rank == 0:
        ...
        logger.info(
            f"Peak normalization: method={args.peak_normalization}, non-zero "
            f"quantile={args.peak_quantile}, fitted per peak column and clipped to [0, 1]; "
    ...
    if rank == 0:
        ...
        logger.info(
            "Token encoding: value + feature identity + primary modality + "
            "partner modality + data form"
        )
        logger.info(
            f"Modalities ({raw_model.modality_count}): "
            + ", ".join(raw_model.modalities)
        )
        if args.mask_strategy == "modality":
            selected_count, masked_count, actual_ratio = (
                raw_model.modality_mask_plan(args.mask_ratio)
            )
            logger.info(
                f"Modality masking: choose {selected_count} modalities per bin, "
                f"mask {masked_count}/{raw_model.feature_count} features "
                f"({actual_ratio:.2%}); requested={args.mask_ratio:.2%}"
            )
        else:
            masked_count = raw_model.feature_count - int(
                raw_model.feature_count * (1 - args.mask_ratio)
            )
            logger.info(
                f"Random masking: mask {masked_count}/{raw_model.feature_count} "
                f"features ({masked_count / raw_model.feature_count:.2%})"
            )
